...
void TournamentController::setGameClubs() {
    ...
    // 随机选择参赛队伍
    std::vector<int> indices(club_repo->getRepo().size() - 1);
    for (size_t i = 0; i < club_repo->getRepo().size() - 1; i++) {
        if (club_repo->getClub(i + 1)->getID() == current_club->getID()) {
            continue;
        }
        indices[i] = club_repo->getClub(i + 1)->getID();
    }
